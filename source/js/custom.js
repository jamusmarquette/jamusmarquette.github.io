...
function onScroll() {
  ...
  const rawProgress = clamp(scrollY / maxScroll, 0, 1);

  const progress = easeOutQuad(rawProgress);

  const value = Math.round(199 + (0 - 199) * progress);

  document.documentElement.style.setProperty('--intro-fade', `rgb(${value}, ${value}, ${value})`);
}
...
if (track) {
  ...
  function advanceWords() {
    ...
    const moveAmount = currentWord.offsetWidth + GAP;
...
document.querySelectorAll('.content a').forEach((link) => {
  link.addEventListener('click', function (e) {
    ...
    content.classList.add('is-clicking');

    setTimeout(() => {
      content.classList.remove('is-clicking');
      content.classList.add('is-releasing');
    }, 120);
