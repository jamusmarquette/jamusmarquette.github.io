---
layout: project

title: CNN Grill at SXSW
page_label: CNN Grill at SXSW

permalink: /project/cnn_grill_at_sxsw/

order: 10

featured: true

categories:
  - Identity & Branding
  - Environmental & Exhibit Design

thumbnail: /project/cnn_grill_at_sxsw/img/01_jamus_marquette_cnn_grill_at_sxsw_signage_event_branding_design.jpg
featured_thumbnail: /project/cnn_grill_at_sxsw/img/02_jamus_marquette_cnn_grill_at_sxsw_signage_event_branding_design.jpg

hero_type: centered-image

hero: /project/cnn_grill_at_sxsw/img/jamus_marquette_cnn_grill_at_sxsw_logo_design.svg


description:
  - A brand identity system unifying CNN’s brand with the SXSW identity to create an experience compelling to tech-savvy attendees for CNN’s pop-up restaurant at Austin’s SXSW Festival.

  - The logo was used as a starting point to develop an engaging tech-savvy language.


credits_title: Collaborators

credits:
  - role: Project Lead
    name: Civic Entertainment Group

  - role: Architectural Plan
    name: DownsArt

  - role: Creative Direction
    name: Sam Eckersley, Stuart Rogers

  - role: Design
    name: Jamus Marquette, Jane Huschka

  - role: Photography
    name: Civic Entertainment Group, Allison Narro

  - role: Designed at
    name: RED Partners

sections:
  - id: concept
    label: Concept
    status: placeholder
  - id: development
    label: Development
    status: placeholder
  - id: system
    label: System
    status: placeholder
  - id: in-use
    label: In Use
    status: complete

media_rows:

  - section: in-use
    caption:
      title: Environmental Graphics System
      text: >
        The visual language extended throughout the venue,
        integrating signage, menus, and interactive moments.

    items:
      - image: /project/cnn_grill_at_sxsw/img/01_jamus_marquette_cnn_grill_at_sxsw_signage_event_branding_design.jpg
        col: col-16 col-md-8

      - image: /project/cnn_grill_at_sxsw/img/02_jamus_marquette_cnn_grill_at_sxsw_signage_event_branding_design.jpg
        col: col-16 col-md-8


  - section: in-use
    items:
      - image: /project/cnn_grill_at_sxsw/img/00_jamus_marquette_cnn_grill_at_sxsw_signage_event_branding_design.jpg
        col: col-16


  - section: in-use
    caption:
      text: >
        Interactive elements, such as the neon boot photo-op,
        encouraged social sharing and engagement with the brand.

    items:
      - image: /project/cnn_grill_at_sxsw/img/04_jamus_marquette_cnn_grill_at_sxsw_signage_event_branding_design.jpg
        col: col-16 col-md-8

      - image: /project/cnn_grill_at_sxsw/img/03_jamus_marquette_cnn_grill_at_sxsw_signage_event_branding_design.jpg
        col: col-16 col-md-8


  - section: in-use
    items:
      - image: /project/cnn_grill_at_sxsw/img/05_jamus_marquette_cnn_grill_at_sxsw_signage_event_branding_design.jpg
        col: col-16 col-md-8

      - image: /project/cnn_grill_at_sxsw/img/06_jamus_marquette_cnn_grill_at_sxsw_signage_event_branding_design.jpg
        col: col-16 col-md-8


  - section: in-use
    items:
      - image: /project/cnn_grill_at_sxsw/img/07_jamus_marquette_cnn_grill_at_sxsw_signage_event_branding_design.jpg
        col: col-16 col-md-8

      - image: /project/cnn_grill_at_sxsw/img/08_jamus_marquette_cnn_grill_at_sxsw_signage_event_branding_design.jpg
        col: col-16 col-md-8


  - section: in-use
    items:
      - image: /project/cnn_grill_at_sxsw/img/09_jamus_marquette_cnn_grill_at_sxsw_signage_event_branding_design.jpg
        col: col-16


  - section: in-use
    items:
      - image: /project/cnn_grill_at_sxsw/img/10_jamus_marquette_cnn_grill_at_sxsw_signage_event_branding_design_neon_boot.jpg
        col: col-16 col-md-8

      - image: /project/cnn_grill_at_sxsw/img/11_jamus_marquette_cnn_grill_at_sxsw_signage_event_branding_design.jpg
        col: col-16 col-md-8


  - section: in-use
    items:
      - image: /project/cnn_grill_at_sxsw/img/12_jamus_marquette_cnn_grill_at_sxsw_signage_event_branding_design.jpg
        col: col-16


  - section: in-use
    items:
      - image: /project/cnn_grill_at_sxsw/img/13_jamus_marquette_cnn_grill_at_sxsw_signage_event_branding_design.jpg
        col: col-16 col-md-8

      - image: /project/cnn_grill_at_sxsw/img/14_jamus_marquette_cnn_grill_at_sxsw_signage_event_branding_design.jpg
        col: col-16 col-md-8


  - section: in-use
    items:
      - image: /project/cnn_grill_at_sxsw/img/15_jamus_marquette_cnn_grill_at_sxsw_signage_event_branding_design.jpg
        col: col-16 col-md-8

      - image: /project/cnn_grill_at_sxsw/img/16_jamus_marquette_cnn_grill_at_sxsw_signage_event_branding_design.jpg
        col: col-16 col-md-8


  - section: in-use
    items:
      - image: /project/cnn_grill_at_sxsw/img/17_jamus_marquette_cnn_grill_at_sxsw_signage_event_branding_design.jpg
        col: col-16


  - section: in-use
    items:
      - image: /project/cnn_grill_at_sxsw/img/18_jamus_marquette_cnn_grill_at_sxsw_signage_event_branding_design.jpg
        col: col-16

---

{% include project/sectioned-media.html %}
